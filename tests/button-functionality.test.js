...
describe('Button Handler Setup - Critical Regression Tests', () => {
  ...
  test('setupButtonHandlers must call setupTimeSaveButtonHandler', () => { expect(setupHandlersCode).toContain('setupTimeSaveButtonHandler'); expect(setupHandlersCode).toMatch(/\\.time-save-btn.*forEach.*setupTimeSaveButtonHandler/); });
  test('setupButtonHandlers must call setupAlertClearedButtonHandler', () => { expect(setupHandlersCode).toContain('setupAlertClearedButtonHandler'); expect(setupHandlersCode).toMatch(/\\.alert-cleared-btn.*forEach.*setupAlertClearedButtonHandler/); });
